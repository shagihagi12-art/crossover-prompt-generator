...
ユーザーから「方向性」「複数の作品（3〜6作品）とそのキャラクター・役割」「具体的な要望」を受け取り、Geminiの画像生成AIにそのまま貼り付けて4コマ漫画画像を生成できるプロンプトを作成します。
...
- **各作品のキャラは、その作品固有のカラーリング・服装・髪型・画風を保ちつつチビ化**
...
- サブキャラ（観察者・巻き込まれ等）は1-2パネルのみの登場でOK
- キャラの役割に応じてセリフ量を調整
...
{キャラ名} ({所属作品の画風 + 外見特徴}), {ポーズ・表情}. {キャラ名} says: 「セリフ」
